...
export default function EmployeeDashboard() {
  ...
  const testRoute = isDemo
    ? "/demo/employee/test/select"
    : "/dashboard/employee/test/select";
  const resultRoute = isDemo
    ? "/demo/employee/results"
    ...
    <div className="space-y-6">
      ...
      {!testStatus.hasCompletedTest && (
        <Card className="border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800">
          ...
          <CardFooter>
            <Button onClick={() => router.push(testRoute)}>
              {t("employee.takeTest")}
            </Button>
          </CardFooter>
        </Card>
          ...
          <Card className="h-full">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                {testStatus.completionDate && (
                  <div>
                    <div className="text-sm font-medium text-muted-foreground mb-1">
                      {t("employee.completionDate")}
                    </div>
                    <div>
                      {new Date(testStatus.completionDate).toLocaleDateString()}
                    </div>
                  </div>
                )}
              </div>
            ...
            <CardFooter>
              {testStatus.hasCompletedTest ? (
                <Button
                  variant="outline"
                  onClick={() => router.push(resultRoute)}
                >
              ...
              {testStatus.hasCompletedTest ? (
                ...
                <div className="flex flex-col items-center justify-center h-32 text-center">
                  <p className="text-muted-foreground">
                    {t("employee.insightsUnavailable")}
                  </p>
                  <p className="text-sm text-muted-foreground mt-2">
                    {t("employee.completeTestForInsights")}
                  </p>
                </div>
              )}
            </CardContent>
