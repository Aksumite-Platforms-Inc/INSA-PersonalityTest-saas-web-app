  ...
  return (
    <div className="container mx-auto py-6">
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground mb-4">{enneagramTest.description}</p>
          {currentQuestions.map((question) => (
            <div key={question.id} className="mb-4">
              <p className="font-medium mb-2">{question.text}</p>
              {question.options.map((option) => (
                <label key={option.value} className="block">
                  <input type="radio" name={`question-${question.id}`} value={option.value} className="mr-2" />
                  {option.label}
                </label>
              ))}
            </div>
        ...
        <CardFooter className="flex justify-between">
          ...
          <Button
            ...
            disabled={(currentGroup + 1) * questionsPerGroup >= questions.length}
          >
            Next
          </Button>
        </CardFooter>
